...
if (isset($_POST['submit'])) {
	...
	if (mysqli_num_rows($sql) > 0) {
		header("location:../employeeadd.php?msg=Email address all ready existed!");
		exit;
	} else {
		if (!empty($_FILES['pfimg']['name'])) {
			...
			if ($type != "image/jpg" && $type != "image/png" && $type != "image/jpeg" && $type != "image/gif") {
				header("location:../employeeadd.php?msg=Invalid image !");
				exit;
			} else {
				if ($size > 1000000) {
					header("location:../employeeadd.php?msg=File size upto 1MB required ! ");
					exit;
				} else {
					$ImageComplete = true;
				}
			...
			if (file_exists("../image/" . $in)) {
				...
				header("location:../employeeadd.php?msg=Pleaes Select Profile Image! ");
				exit;
			}
		}
	}
	...
	if ($ImageComplete) {
		...
		if ($editid == 0) {
			...
			header("location:../detailview.php?id=Successfull... ");
			exit;
		} else {
			if (!empty($_FILES['pfimg']['name'])) {
				$name = rand(222, 333333) . $name;
			...
			header("location:../detailview.php?employeeid=$editid");
			exit;
		}
